...
class TemporalEmbeddingMicroseconds(nn.Module):
    def __init__(self, d_model, embed_type='fixed', freq='h'):
        super(TemporalEmbeddingMicroseconds, self).__init__()

        microseconds_size = 2000
        milliseconds_size = 2000
    ...
    def forward(self, x):
        x = x.long()
        milliseconds_x = self.milliseconds_embed(x[:, :, 0])
        microseconds_x = self.microseconds_embed(x[:, :, 1])
        return milliseconds_x + microseconds_x
...
class DirectionEmbedding(nn.Module):
    def __init__(self, d_model, embed_type='fixed', freq='h'):
        ...
        direction_amount = 3  # 2 + 1 no direction
...
class DataEmbedding_w_dir_temp(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.temporal_embedding = TemporalEmbeddingMicroseconds(d_model=d_model, embed_type=embed_type,
                                                                freq=freq)
        self.dropout = nn.Dropout(p=dropout)
